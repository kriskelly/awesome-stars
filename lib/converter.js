  ...
  convertListToTable: function(tree) {
    ...
    function traverse(root) {
      if (Array.isArray(root)) {
        ...
        if (root.type === 'loose_item_start') {
          // Really not sure why loose_item_start needs to be here
          // in the first place, but removing them un-breaks
          // the parsing.
          removeElementFromTree(root);
        } else if (root.type === 'list_start') {
          listStarted = true;
          listItems = [];
          removeElementFromTree(root);
